...
| Shift Type Successions | `/shift-type-successions` | `shift type successions` | CON-SEM |
...
- **FR-PR-14 — Empty-dependency guidance.** Any selector whose underlying entity
  ...
  setup tab, verbatim: `No shift types available. Please set up shift types in the
  Shift Types tab first.`; `No people available. Please set up people in the
  People tab first.`; `No dates available. Please set up dates in the Dates tab
  first.` (requirements:593-600,740-747,779-786; successions:394-401,432-439,543-550; counts:418-425,457-464,496-503; affinities:318-325,357-364,395-402,433-440)
...
- **FR-PR-32 — Ordered pattern by appending.** `Shift Type Pattern * (click to
  add shift types)` presents every shift type item and group (including `OFF` and
...
- **FR-PR-82 — Preceptees multi-select (required).** `Preceptees (must be
  covered) *` is a multi-select over people items + groups. Stored as
...
- **FR-PR-86 — Weight parsing and semantics.** Weight is parsed with the
  ...
  weight. **The current backend (`core/nurse_scheduling/preference_types.py:622-633,
  :701-721`) does not read `preference.weight` at all** — every valid
...
- **FR-PR-89 — Editor parity with other card editors.** The covering editor
  ...
  field with placeholder `e.g., Lil must always be paired with Anna on Day
  shift` (coverings:306); global Enter=save / Escape=cancel under the
...
- **FR-PR-71 — Empty state.** When no coefficient-eligible shift type is selected,
  the sub-editor shows italic text `Coefficients are not needed when no
  <singular-label> is selected.` where `<singular-label>` is the lowercased label
...
- **FR-PR-41 — Undefined staffing warning.** For each staffed shift-type item,
  ...
  at least one missing date are listed. The banner header is `Requirement coverage
  warnings`. The undefined section text is:
  `Undefined staffing requirements: <N> date/shift type pairs have no
  requirement, so the solver may assign an arbitrary number of people.` followed
...
- **FR-PR-42 — Duplicate staffing warning.** When a pair is covered more than
  ...
  `Duplicate staffing requirements: <M> date/shift type pairs are covered by more
  than one requirement. The solver will apply all matching requirements.`
