...
Underlying data type: `ShiftRequestPreference = { type: SHIFT_REQUEST, person: string[], date: string[], shiftType: string[], weight: number }. `**Editor-created and**
...
**and modal stays open) if any draft weight is invalid; the guard string is **`"Weight must be a valid number, Infinity, or -Infinity" (ShiftPreferenceEditor.tsx:87,95).`
...
the operation is skipped with a console warning `"Cannot set history to a shift type group." (no user-facing message). `**Selecting more than one shift type sets the error**
...
is invalid (`!isValidWeightValue(addFormData.weight)); its tooltip then reads "Weight must be a valid number, Infinity, or -Infinity", otherwise "Upload a CSV file with shift preferences (people x (dates + 1) matrix)". The People History tooltip is`
...
when positive) and a neutral/wants/avoid caption (page.tsx:1912-1971). Empty state: `"No shift requests defined yet. Click on any cell in the matrix above to add preferences."`
...
the fixed type order `[at most one shift per day, shift type requirement, shift request, shift type successions, shift count, shift affinity, shift type covering]`
