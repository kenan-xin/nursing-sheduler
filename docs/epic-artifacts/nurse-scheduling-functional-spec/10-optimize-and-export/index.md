...
`lockedOptimizeEndpoint ?? resolvedServer.endpoint ?? serverEntries[0].endpoint ?? '' (page.tsx:471-474).`
...
shown as: `API version: <apiVersion ?? version> · Frontend version: <CURRENT_APP_VERSION> · Backend version: <appVersion> (page.tsx:1386-1388).`
...
`"Frontend and backend versions do not match. If nothing breaks, you can continue." (page.tsx:1389-1393). It does not disable optimization.`
...
Label `"Anonymize schedule data", help "Anonymize people IDs and remove descriptions before sending to the backend." (page.tsx:436, 1428-1439).`
...
online or the resolved endpoint is empty, set error `"Select an online backend before optimizing." and return (page.tsx:795-799).`
...
a chart point `{ currentBestScore, elapsedSeconds, commentCount, solutionIndex, source } (page.tsx:321-323, 724-742).`
...
polled: `GET buildApiUrl(runEndpoint, job.links.status) with cache: 'no-store' every `**1000 ms until a terminal status; each poll updates the**
...
thrown. If `xlsxReady is false, throw No downloadable schedule is available. Job status: <status> (page.tsx:849-865).`
...
`POST buildApiUrl(runEndpoint, currentJob.links.heartbeat ?? '/optimize/<jobId>/heartbeat') with cache: 'no-store'; errors are swallowed`
...
`"Incumbent Progress" and "Higher scores are better. Hover to inspect a solution." (OptimizationProgressChart.tsx:216-219).`
...
`"<Solution #<idx>|Incumbent> · <elapsed|time unavailable> · <n comments|comments unavailable>[ · <source>]"; else "Job <jobId>". When a job id exists,`
`"Job ID: <jobId>" is also shown (page.tsx:1536-1559).`
- **FR-OE-68 (XLSX fetch) — On success, **`GET buildApiUrl(runEndpoint, completedJob.links.xlsx); a non-OK response throws Server error (<status>): <detail> (page.tsx:867-873).`
- **FR-OE-69 (ID restoration) — When the run was anonymized, the downloaded XLSX**
is post-processed by `restorePeopleIdsInXlsx(blob, originalIdByAnonymizedId, peopleCount) before download; when not anonymized the raw blob is used`
...
message to `error.message, or "An unexpected error occurred during optimization" for non-Error throws (page.tsx:896-902).`
...
surfaces as the error message, and `xlsxReady === false yields No downloadable schedule is available. Job status: <status>.`
...
is deleted via `DELETE /optimize/<jobId>, and the success message is Schedule optimized and downloaded successfully!.`
