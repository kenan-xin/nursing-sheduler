...
**FR-EX-01 — ExportConfig shape. The export config is **`{ formatting?, extraColumns?, extraRows? }; all three keys are optional arrays`
...
type is exactly one of `'cell' | 'row' | 'column' | 'people header' | 'date header' | 'history' | 'history header' (scheduling.ts:21-28,84-88). All rules share the`
...
`appendText is written only if non-empty and note is written as { text: noteText } only if the trimmed note text is non-empty (page.tsx:554-555).`
...
when at least one condition input is set: `hasCondition = requestShape.length > 0 || satisfied !== '' || (weightRangeMin or weightRangeMax non-blank)`
...
`{ description?, rightBorderColor?, type: 'count', header, countShiftTypes: string[], countShiftTypeCoefficients?: ShiftCountTypeCoefficient[], countDates: string[] } (scheduling.ts:90-98). Semantically it produces a per-person summary`
...
`{ description?, bottomBorderColor?, type: 'count', header, countShiftTypes: string[], countPeople: string[] } (scheduling.ts:100-107). Semantically it`
...
effective config used by the editor is `generateExportLayoutConfig(shiftTypes, dates.groups) (useSchedulingData.ts:972-973); the same fallback is applied`
...
`shiftTypes: [ALL, OFF], when.preference = { types: ['shift request'],  requestShape: ['person-item-to-date-item'], weightRange: [-Infinity, Infinity] }`
(`schedulingExportConfig.ts:159-173).`
2. `cell — description "Mark unsatisfied shift requests",`
`appendText: " [X]", fontColor: '#c00000', note.text = "Weight of unmet  single-style request: {totalAbsWeight}", people: [ALL], dates: [ALL],`
`shiftTypes: [ALL, OFF], when.preference = { types: ['shift request'],  requestShape: ['person-item-to-date-item'], satisfied: false, weightRange:  [-Infinity, Infinity] } (schedulingExportConfig.ts:174-193).`
...
7. (only when a `NON-WORKDAY date group exists) column — description "",`
`dates: [NON-WORKDAY], backgroundColor: '#dcfce7'`
(`schedulingExportConfig.ts:217-222).`

`ALL, OFF, WEEKDAY, WEEKEND, SATURDAY, SUNDAY are reserved keyword ids`
(`keywords.ts:23-33); WORKDAY/NON-WORKDAY are the workday/non-work day group ids`
...
3. `OFF (NON-WORKDAY) — present only when a NON-WORKDAY date group exists;`
`countShiftTypes: [OFF], countDates: [NON-WORKDAY]`
(`schedulingExportConfig.ts:145-151).`
The last of the (WORKDAY, NON-WORKDAY) columns, if any, gets
...
- **WORKDAY/NON-WORKDAY/NONWORKDAY-column conditionals. The **`OFF (WORKDAY) and`
`OFF (NON-WORKDAY) columns and the NON-WORKDAY background column appear in the default`
...
to the *last item of a generated run (last WORKDAY/NON-WORKDAY column, last*
...
including the seven ordered default formatting rules (or six when no NONWORKDAY
...
`when, when saved, then when.preference equals { types: ['shift request'], satisfied: false } with no requestShape or weightRange keys.`
...
- **Shift Requests spec. **`ExportRequestShape and the when.preference.types = ['shift request'] condition tie directly to the shift-request preference model;`
...
`NON-WORKDAY (keywords.ts, schedulingConstants.ts) are defined by those specs.`
