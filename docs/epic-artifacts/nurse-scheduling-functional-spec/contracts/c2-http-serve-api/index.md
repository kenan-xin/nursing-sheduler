...
domain: HTTP serve API (contract)
prefix: CON-API
fidelity: STRICT PARITY (conformance-only)
sources:
  - core/nurse_scheduling/serve.py
  - core/nurse_scheduling/jobs.py
---
...
- **Response 202** (`status_code=202`, serve.py:454): a **job-response object**
  (see [Job-Response Object](#job-response-object)) for the freshly created
  `QUEUED` job.
...
- **Response 200:**
  ...
  ```
  (serve.py:511 — note: this is a **reduced** shape, not the full job-response object.)
...
- **`complete`** — payload is the full [job-response object](#job-response-object) at terminal time. Emitted for any terminal status other than `FAILED` (`_finish_optimize_job_locked` appends `{"event": "complete", ...}`, jobs.py:317; synthetic fallback serve.py:396-397).
