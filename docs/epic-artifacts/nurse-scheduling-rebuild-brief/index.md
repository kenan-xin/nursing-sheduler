...
flowchart LR
  subgraph FE["web-frontend — Next.js SPA (BEING REBUILT)"]
    UI["13 tabs: Home, Dates, People, Shift Types,\n6 preference editors (incl. Coverings),\nExport Layout, Save/Load, Optimize & Export"]
  ...
  subgraph BE["core — Python (FIXED CONTRACT)"]
    ...
    ENG["Constraint model + OR-Tools CP-SAT\n(only solver — fixed contract)"]
...
| 2 | **Dates & calendar** | Range-driven date generation; `DD / MM-DD / YYYY-MM-DD ID formatting by span; Singapore-holiday import (English-only); date groups.` |
| 3 | **People & Shift Types editors** | Item/group CRUD, inline edit, reorder, duplicate, bulk people upload, reserved-keyword rules. |
| 4 | **Preference editors (×6)** | Shift Requests (matrix + quick-add + CSV), Shift Type Requirements, Shift Type Successions, Shift Counts, Shift Affinities, Shift Type Coverings — fields, validation, weight semantics. |
