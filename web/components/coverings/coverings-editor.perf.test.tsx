...
//
// Perf guard for the Shift Coverings editor's store subscription (zqu, the b8z
...
// and re-ran its derivations (the covering card list, the shared people/shift-type
// option builders, the date scopes, the covering form validation). It now reads
// exactly the slices it derives from, so an edit elsewhere must leave it alone.
//
...
describe("CoveringsEditor — narrowed store subscription", () => {
  it("does not re-render on a mutation to a slice the screen never reads", async () => {
    render(<Tracked />);
    const before = commits;

    // `meta` belongs to the shell's scenario name; Coverings never reads it.
