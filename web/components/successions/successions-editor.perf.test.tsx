...
//
// Perf guard for the Shift Successions editor's store subscription (zqu, the b8z
...
// and re-ran its derivations (the succession card list, the shared transfer-option
// builders, the date scopes). It now reads exactly the slices it derives from, so
// an edit elsewhere must leave it alone.
//
...
describe("SuccessionsEditor — narrowed store subscription", () => {
  it("does not re-render on a mutation to a slice the screen never reads", async () => {
    render(<Tracked />);
    const before = commits;

    // `meta` belongs to the shell's scenario name; Successions never reads it.
