...
describe("PeopleTable — no temporary flag (d582)", () => {
  it("has no Temporary switch or badge", async () => {
    await seed({ staff: [{ id: "Float", history: [] }], staffGroups: [] });
    render(<PeopleTable />);
    expect(screen.queryByText("Temporary")).not.toBeInTheDocument();
    fireEvent.click(screen.getByTestId(`people-edit-${sk("Float")}`));
    expect(screen.queryByRole("switch", { name: /temporary/i })).not.toBeInTheDocument();
    fireEvent.click(screen.getByTestId("people-add"));
    expect(screen.queryByRole("switch", { name: /temporary/i })).not.toBeInTheDocument();
  });
});
