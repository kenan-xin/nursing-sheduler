...
// never hydrates. Feeding this machine's own non-internal IPv4 addresses to
// `allowedDevOrigins` (development only) is what lifts that block.
...
/**
 * The hostnames `next dev` must accept dev requests from on this machine: every
 * non-internal IPv4 address, de-duplicated. Loopback and IPv6 entries are left
 * out — loopback is already a Next default, and a browser origin carries a bare
 * IPv4 literal, not a bracketed IPv6 one.
 *
...
export function getAllowedDevOrigins(
  ...
  const addresses = Object.values(interfaces).flatMap((entries) =>
    (entries ?? [])
