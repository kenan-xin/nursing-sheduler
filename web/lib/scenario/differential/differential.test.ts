...
const PROBE = probeBackend({ op: "shift_map", items: ["D"], groups: [] });
...
describe.skipIf(!AVAILABLE)("differential — C1 (bytes → load_data, exact model dump)", () => {
  it(
    "accepts the serialized fixture and canonicalizes (zero-rest omitted, implicit ALL)",
...
`;
  ...
  it(
    "rejects a fractional person id and an impossible date (declared: ValidationError)",
...
preferences: [{type: at most one shift per day}]
`;
      expect(callOracle({ op: "load", yaml: badId }, "fractional person id").ok).toBe(false);
...
preferences: [{type: at most one shift per day}]
`;
      expect(callOracle({ op: "load", yaml: badDate }, "impossible date").ok).toBe(false);
    },
  );
  ...
  it(
    "rejects working-time whole shapes (declared: ValidationError)",
    { timeout: oracleBudget(5) },
    () => {
...
`;
      ...
      for (const [name, yaml] of Object.entries(cases)) {
        expect(callOracle({ op: "load", yaml }, name).ok).toBe(false);
      }
    },
  );
});
...
describe.skipIf(!AVAILABLE)("differential — C3 (scheduler/context setup + group_map)", () => {
  ...
  it(
    "mixed-group→LEAVE request: producer rejects, backend silently schedules OPTIMAL",
    { timeout: oracleBudget(1) },
    () => {
      const state = makeValidUiState();
  ...
  it(
    "rejects OFF in a shift type requirement (declared: ValueError)",
    { timeout: oracleBudget(1) },
    () => {
      const yaml = `apiVersion: alpha
dates: {range: {startDate: 2026-05-14, endDate: 2026-05-14}}
...
`;
  ...
  it(
    "catches an unknown selector that zod preflight accepts (zod-pass ≠ backend-accept)",
    { timeout: oracleBudget(1) },
    () => {
      const state = makeValidUiState();
      state.cardsByKind.requirements = [
  ...
  it("distinguishes date omitted (all) from date:[] (no-op)", { timeout: oracleBudget(2) }, () => {
...
${dateLine}    weight: .inf
`;
    expect(callOracle({ op: "schedule", yaml: body("") }, "date omitted").status).toBe(
      "INFEASIBLE",
    );
  ...
  it(
    "contracted-hours incomplete coverage: producer rejects and backend rejects at load",
    { timeout: oracleBudget(1) },
    () => {
      const state = makeValidUiState();
...
describe.skipIf(!AVAILABLE)("differential — qq0.23a (lossless people/date resolution)", () => {
  ...
  it(
    'people map: numeric 1 vs string "1" stay two distinct tagged records',
    { timeout: oracleBudget(1) },
    () => {
  ...
  it(
    "people resolution: numeric and string selectors resolve independently",
    { timeout: oracleBudget(2) },
    () => {
      const staff = [1, "1"] as const;
      ...
      for (const [selector, expected] of [
        ...
        const backend = callOracle(
          { op: "resolve_people", staff: [1, "1"], groups: [], selector },
  ...
  it(
    "people resolution: an unknown id and a forward-group both fail closed",
    { timeout: oracleBudget(2) },
    () => {
      const unknown = callOracle(
        { op: "resolve_people", staff: ["A"], groups: [], selector: "Z" },
        "unknown id",
      );
      ...
      const forward = callOracle(
        { op: "resolve_people", staff: ["A"], groups: forwardGroups, selector: "g" },
        "forward group",
      );
  ...
  it(
    "date resolution: direct key, numeric fallback, ranges, keywords, and failures",
    { timeout: oracleBudget(8) },
    () => {
      const ctx = buildScenarioResolutionContext({
        staff: [],
        staffGroups: [],
  ...
  it(
    "year-zero range fails closed on both sides without crashing the oracle",
    { timeout: oracleBudget(1) },
    () => {
  ...
  it(
    "date group: direct-key precedence before parse, then group is addressable",
    { timeout: oracleBudget(1) },
    () => {
...
describe.skipIf(!AVAILABLE)("differential — C5 (exporter, exact workbook)", () => {
  it(
    "produces exact cells, a painted cell fill, frozen panes, and export notes",
    { timeout: oracleBudget(1) },
    () => {
      const yaml = `apiVersion: alpha
dates: {range: {startDate: 2026-05-14, endDate: 2026-05-14}}
...
`;
  ...
  it(
    "rejects an uncovered export extra-column coefficient (declared: ValueError)",
    { timeout: oracleBudget(1) },
    () => {
      const yaml = `apiVersion: alpha
dates: {range: {startDate: 2026-05-14, endDate: 2026-05-14}}
...
describe.skipIf(!AVAILABLE)("differential — anonymized path", () => {
  it(
    "an anonymized document (incl. a person named OFF) still loads and carries P#/G# ids",
    { timeout: oracleBudget(1) },
    () => {
      const state = makeValidUiState();
  ...
  it(
    "co-derived submission YAML loads in the backend and its reverse map restores the originals",
    { timeout: oracleBudget(1) },
    () => {
      const state = makeValidUiState();
  ...
  it(
    "co-derived anonymized submission loads in the backend with all descriptions stripped",
    { timeout: oracleBudget(1) },
    () => {
      const state = makeValidUiState();
...
`,
    ...
    for (const [name, raw] of Object.entries(FIXTURES)) {
      it(
        `raw → import → normalize → canonical → producer → YAML ≈ raw (${name})`,
        { timeout: oracleBudget(1) },
        () => {
          ...
          const res = callOracle({ op: "roundtrip", raw, roundtrip: roundtripYaml }, name);
          expect(res.ok).toBe(true);
