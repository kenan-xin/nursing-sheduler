...
//   • roster       — `rosterStorage` (F1) and the `getRosterDb` accessor, both now
//                    sitting on the ONE merged Dexie ladder rather than a second
//                    schema owner.
//
...
// F1 roster storage: typed, transactional repositories for submission snapshots,
// candidates, and the working roster. Unchanged by the integration — it sits ON the
// single merged Dexie ladder rather than owning a second one, which is the whole
// point of `dexie-storage.ts` above being an accessor and not a schema.
export {
  candidateRosterKey,
  createRosterStorage,
