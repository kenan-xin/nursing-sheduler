...
export type OptimizeObservation =
  ...
  // `retained` is a DELIBERATE non-deletion. An infeasible run's server record is
  // kept as the bounded evidence T10's diagnostic classifies against, while the run
  // is retired locally. Emitting it distinctly keeps "we chose to keep this"
  // separable from "we failed to delete it" in the cleanup telemetry.
  //
  // INTEGRATION: `abandoned` is NOT in this union. Visit scoping made cleanup
  // invisible and removed the user-facing abandon surface, so nothing emits it any
  // more; carrying a member no producer can emit would misdescribe the surface.
  | { kind: "cleanup"; jobId: string; result: "cleaned" | "failed" | "retained" }
