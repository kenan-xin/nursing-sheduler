...
// ---------------------------------------------------------------------------
// REMOVED (G6.2) — the page-plane recovery-notice ladder + prototype-deviation
// measurements that stood here.
//
// They drove the REAL route into its two neutral recovery states (a seeded
// resumable record, and a denied read of that record's key) and measured the
// notice's plane against DESIGN.md §4 and the prototype's flat `--panel` strip.
// Neither state can be reached now: entering the route inspects no record, so
// there is nothing to resume and no boot read to fail, and `RecoveryNotice` is
// deleted. The route has no neutral page-plane notice left to measure.
//
// The RULE they defended is still enforced, one level down: `optimize-v2-roles`
// asserts that a page-placed neutral `Callout` resolves the L1 surface role and
// not the unhosted well. What is lost is the browser-resolved measurement of
// that rule against the prototype — recorded as a coverage delta rather than
// quietly dropped.
