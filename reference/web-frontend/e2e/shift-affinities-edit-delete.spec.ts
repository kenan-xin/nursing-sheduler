...
test('shift affinities can be edited and deleted through the page UI', async ({ page }) => {
  /*
   * Steps:
   * 1. Open shift affinities with one seeded rule and confirm its original values.
   * 2. Edit the rule through the page UI and confirm the updated selections appear.
   * 3. Delete the same rule and confirm the list returns to the empty state.
   */
  await disableModalDialogs(page);
  await seedSchedulingState(page, {
    apiVersion: 'test',
    description: 'shift affinity edit seed',
    dates: {
    ...
    preferences: [
      ...
      { type: 'shift affinity', date: ['01'], people1: ['P1'], people2: ['P1'], shiftTypes: ['D'], weight: 1, description: 'affinity rule' },
    ],
    export: { formatting: [] },
  });
  ...
  await expect(page.getByText('People 2: P1, P2')).toBeVisible();
