...
| Collapsible desktop sidebar and `ns-side-collapsed` persistence | **Shipped (G8).** The user superseded the prior exclusion. At `≥920px` the rail toggles between the production **280px** expanded panel and a **60px** compact icon rail, persisted under `ns-side-collapsed`. The expanded width stays 280 (the prototype's 244 would have been a separate shell re-layout); 60 is the prototype's literal rail. The mobile drawer always renders expanded. | In scope. See the navigation contract in §5 for the full rule set. |
...
### Toast
- Bottom-centre, `--ink` fill, `--on-ink` text, leading ✓ mark, the `--sh-3` modal shadow, click to dismiss, `role="status"`.
...
- **Desktop collapse (≥920px).** The rail toggles between `--sidebar-w` **280px** and `--sidebar-w-compact` **60px**, animated on `width` over `--dur-base`. The control is the shared 36px `ghost`/`icon` button at the far left of the top bar (`fa-angles-left` / `fa-angles-right`), carrying `aria-expanded` and `aria-controls` on the rail plus a `title`/`aria-label` naming the ACTION. The preference persists under `ns-side-collapsed` (`"1"` collapses; anything else, including a throwing storage, stays expanded).
  - The width authority is `data-side-collapsed` on `<html>`, written pre-paint by an inline script and rewritten by the store on every toggle, so a reload paints the compact rail directly. That rule is **unlayered** in `globals.css` — inside `@layer base` it loses to the unlayered `:root` regardless of specificity. `data-side-ready` releases a `visibility` hold on the rail's interior once React has adopted the preference, so the pre-hydration frame is blank rather than visibly clipped.
  - Compact rules: 10px gutters; brand mark only, with the product name on `title` and an sr-only copy; nav items become 40×38 centred icon buttons at the control radius, `--brandtint` for the active one, each with a `title` **and** a matching `aria-label` (`"Dates · step 1"`, the ordinal only in Guided) plus `aria-current="page"`; group headings become 1px `--line2` separators that keep their heading as `role="separator"` + `aria-label`; the Guided/Advanced segmented control folds into one `GUI`/`ADV` pill that routes through the same guarded mode transaction; the footer keeps only the theme control and drops the two-line identity well, which cannot truncate legibly at 60px.
  - Collapsing never removes a destination, an accessible name, or the active signal. The mobile drawer is passed `collapsed={false}` **literally** and is never affected by the desktop preference.
